...
```{r Ex 8.1 load data, message=FALSE, echo = TRUE, warning=FALSE, message= FALSE}
...
```{r Ex 8.1 nimble set constants and inits, error = FALSE, message = FALSE, results='hide'}
...
```{r Ex 8.1 nimble WAIC, ESS and traceplots}
...
```{r Ex 8.1 stan load, message=FALSE, echo = TRUE, warning=FALSE, message= FALSE}
...
```{r Ex 8.3 nimble load, message=FALSE, echo = FALSE, warning=FALSE, message= FALSE}
rm(list=ls())
...
```{r Ex 8.3 nimble munge functions, echo = TRUE}
...
```{r Ex 8.3 nimble car model, warning=FALSE}

CAR_Code <- nimbleCode({
  # Likelihood
  for (i in 1:N) {
    y[i] ~ dpois(lambda[i])
    log(lambda[i]) <- log(E[i]) + b[i] + beta0
  }
  
  # Priors
  beta0 ~ dnorm(0, sd = 0.98)
...
```{r Ex 8.3 nimble car run, error = FALSE, message = FALSE}
# constants list
constants = list(
  N = N,
  E = expected$E2010,
...
# QUESTION: can I run this model without configuring it as I did in the previous examples? 
CAR_conf <-
  configureMCMC(CAR_Model, monitors = c("beta0", "fitted", "b", "sigma_b"))
CAR_MCMC = buildMCMC(
  CAR_Model,
  enableWAIC = T,
  monitors = c("beta0", "fitted", "b", "sigma_b")
)
...
```{r Ex 8.3 WAIC, ESS and traceplots }

CAR_runMCMC$WAIC
min(coda::effectiveSize(CAR_runMCMC$samples))

plot(CAR_runMCMC$samples[, c("beta0")])
plot(CAR_runMCMC$samples[, c("b[2]")])
plot(CAR_runMCMC$samples[, c("sigma_b")])
```


```{r Ex 8.3 nimble CAR summary, echo = TRUE, results='asis'}

# Extract samples

variables <- c("beta0", "sigma_b")
summary_CAR_nimble <- CAR_runMCMC$summary$all.chains

knitr::kable(summary_CAR_nimble[variables,])
...
samples_CAR_b <- summary_CAR_nimble[grepl("b\\[", rownames(summary_CAR_nimble)),] %>% as.data.frame()
...
```{r Ex 8.3 stan car effects, warning=FALSE}
...
COPD_fitCAR  <- stan(
  file = "functions/Example8_3.stan",
  data = ex.data,
  warmup = 10000,
  iter = 20000,
  chains = 2,
  thin = 10,
  pars = c("beta0","sigma_s", "b", "log_lik"),
  include = TRUE
)
...
loglikcar <- extract_log_lik(COPD_fitCAR)
...
knitr::kable(summary_CAR_stan$summary)
...
```{r Ex 8.4 carbayes results, results='asis'}
