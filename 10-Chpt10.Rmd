...
Mtl_centroids_utm <- spTransform(Mtl_centroids,
                        CRS("+proj=utm +zone=18 +ellps=WGS72"))
plot(Mtl_centroids_utm, pch = 16, cex = 0.5)
# Save the utm as a data frame
...
spat.pred.nimble <-
  function(coords,
    ...
    mu <-
      beta0_post + beta1_post * coords[,"easting"] + beta2_post *
      coords[,"northing"]
    
    mu_u <-
      beta0_post + beta1_post * pred_coords[, "easting"] + beta2_post *
      pred_coords[, "northing"]
    
    all_dist <- rdist::rdist(pred_coords, coords)
    
    Sigma <- tau_post ^ 2 * diag(pred_coords + coords) +
      sigma_post ^ 2 * exp(-all_dist / phi_post)
    ...
    Sigma_pred <- Sigma_unobs-Sigma_unobs_obs %*% solve(Sigma_obs) %*% t(Sigma_unobs_obs)
...
do.call("mapply", c(list(funcion_prueba), mcmc_samples))
