...
```{r Example 10.10 spatial predictions nimble, message=FALSE, cache = TRUE}
...
Example10_10Code <-  nimbleCode ({
  ...
  for (usite in 1:nu) {
    mean.unobs.site[usite] <-
      beta0 + beta1 * easting_unobs[usite] + beta2 * northing_unobs[usite]
  }
  
  mean_sigma[1:nu] <- t(Sigma_obs_unobs[1:n, 1:nu])%*% 
    inverse(Sigma_obs[1:n, 1:n])%*%(y[1:n] - mean.site[1:n])
  
  mu_unobs[1:nu] <-
    mean.unobs.site[1:nu] + mean_sigma[1:nu]
  
  cov_unobs[1:nu, 1:nu] <-
    Sigma_unobs[1:nu, 1:nu] - t(Sigma_obs_unobs[1:n, 1:nu]) %*% inverse(Sigma_obs[1:n, 1:n]) %*% Sigma_obs_unobs[1:n, 1:nu]
  
  y_unobs[1:nu]  ~  dmnorm(mu_unobs[1:nu], cov = cov_unobs[1:nu, 1:nu])
...
ex.data <-
  list(
    ...
    easting = easting_scaled,
    northing = northing_scaled,
    easting_unobs = predCoords_five[,1],
    northing_unobs = predCoords_five[,2],
    distMatrix = obsDist,
