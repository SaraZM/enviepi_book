...
```{r Ex 10.8 nimble predictions using Rcpp}
...
traceplot(Example10_9Stan, pars = c("beta0","beta","sigma_sq", "tau_sq", "phi"))
...
# Extract samples from nimble model
stan_post_samples <- extract(Example10_9Stan,
    pars = c("beta0","beta", "phi", "sigma_sq", "tau_sq"))
...
```{r Ex 10.14 load libraries and data, message = FALSE, warning=FALSE, error=FALSE, eval = FALSE}
...
```{r Ex 10.14 explore data, eval = FALSE}
head(gambia)
dim(gambia)
...
```{r Ex 10.14 aggregate data, eval = FALSE}
...
```{r Ex 10.14 plot prevalence, eval = FALSE}
...
```{r Ex 10.14 load covariates and plot altitude, warning=FALSE,eval = FALSE}
...
```{r Ex 10.14 extract altitude at locations, eval = FALSE}
...
```{r Ex 10.14 nimble model, eval = FALSE}
...
```{r Ex 10.14 run nimble model, warning = FALSE, error = FALSE, message = FALSE, results='hide', eval = FALSE}
...
```{r Ex 10.14 nimble traceplots ESS, eval = FALSE }
...
```{r Ex 10.14 nimble summary, echo = TRUE, eval = FALSE}
...
```{r Ex 10.14 nimble summary p, echo = TRUE, eval = FALSE}
...
```{r Ex 10.14 nimble summary S, echo = TRUE, eval = FALSE}
...
```{r Ex 10.14 load stan, message=FALSE, echo = FALSE, warning=FALSE, message= FALSE, eval = FALSE}
# load rstan with options
library(rstan)
options(mc.cores = parallel::detectCores())
rstan_options(auto_write = TRUE)
...
```{r Ex 10.14 stan data, warning = FALSE, eval = FALSE}
...
```{r Ex 10.14 stan traceplots, eval = FALSE}
...
```{r Ex 10.14 stan summary, echo = TRUE, eval = FALSE}
...
```{r Ex 10.14 stan posterior plots, eval = FALSE}
S_summary <-
