...
Example10_14_Nimble <- nimbleCode({
  ...
  # 
  # Sigma[1:N, 1:N] <-
  #    sigma_sq*(1 + (sqrt(3)*obs_dist_mat[1:N, 1:N])/phi) * exp(-sqrt(3)*obs_dist_mat[1:N, 1:N] / phi) + tau_sq  * identityMatrix(d = N)
  # 
  # S[1:N] ~ dmnorm(zeroes[1:N], cov = Sigma[1:N,1:N])
  # 
    for (j in 1:n) {  # by child
    logit(p[j])  <-  b0 + inprod(b[1:k], X[j,1:k]) 
    #+ S[index_village[j]]
...
dat_list <- list(y = gambia_df$pos, # malaria positive test 
                 ...
                 X = gambia_df[,4:8], # predictors matrix
                 index_village = index_village
                 )

# init_list <- list(tau = 0.01, sigma = 0.01, p = rep(0.01, n), phi_inv = 6/max(obs_dist_mat))

init_list <- list(p = rep(0.01, n))
...
conf <-
  configureMCMC(Rmodel, monitors = c( "b0", "b", "p"))
...
# Extract samples
variables <- c("b0", "b[1]", "b[2]", "b[3]", "b[4]", "b[5]")
...
Example10_14Stan  <- stan(
  ...
  chains = 2,
  thin = 10,
  pars = c("beta0", "betas","sigma", "tau", "phi", "S"),
  include = TRUE
)
...
rstan::traceplot(Example10_14Stan, pars = c("beta0","betas","sigma", "tau", "phi"))
...
summary_stan <-
  summary(
    Example10_14Stan,
    pars = c("beta0","betas", "tau", "sigma", "phi"),
    probs = c(0.025, 0.975)
  )
