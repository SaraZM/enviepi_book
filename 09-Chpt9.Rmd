...
CAR_conf <-
  configureMCMC(CAR_Model, monitors = c("beta0","beta1", "fitted", "b", "sigma_b"))
CAR_MCMC = buildMCMC(
  CAR_Model,
  enableWAIC = T,
  monitors = c("beta0", "beta1","fitted", "b", "sigma_b")
)
...
plot(CAR_runMCMC$samples[, c("beta0")], bty = "n")
plot(CAR_runMCMC$samples[, c("beta1")], bty = "n")
plot(CAR_runMCMC$samples[, c("b[2]")], bty = "n")
plot(CAR_runMCMC$samples[, c("sigma_b")], bty = "n")
```
...
library(rstan) 
library(spdep) 
# Function to clear the axes of the ggplot map 
theme_clear <- function(){
  theme(
    panel.grid.major = element_blank(), # remove background grid
    panel.grid.minor = element_blank(),
...
traceplot(COPD_fitCAR, pars = c("beta0","beta","sigma_s"))
...
SMR2010 <- model$fitted.values /  copd_df$E2010
