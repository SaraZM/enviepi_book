...
### Exercise 13.16-13.18: Age-related causal effects

#### 13.16: Load the Italy and China data, described in Example 13.3, and included in the supplementary Bookdown material into R.
...
vonK_Data_China <- read_csv("data/vonK_China_Italy.csv") 
...
```{r Exercise 13.16 1a), message=FALSE, echo = TRUE, warning=FALSE, message= FALSE}

## Sum up all deaths by country

vonK_Aggregated_Table <- vonK_Data_70 |> select(-Age_Range) |> group_by(Country) |> summarise_all(.funs = c(sum)) |> adorn_totals("row")


knitr::kable(vonK_Aggregated_Table, caption = "Aggregated Contingency Table")

```

##### ii): Display 2 by 2 contingency tables stratified by the age groups $<70$ and $\geq 70$. 

```{r, Exercise 13.16 1b), message=FALSE, echo = TRUE, warning=FALSE, message= FALSE}

## Filter by age range and then sum up deaths

vonK_Under70_Table <- vonK_Data_70 |> filter(Age_Range == "Under_70") |> select(-Age_Range) |> adorn_totals("row") 
vonK_Over70_Table <- vonK_Data_70 |> filter(Age_Range == "Over_70") |> select(-Age_Range) |> adorn_totals("row")

knitr::kable(vonK_Under70_Table, caption = "Under 70 Contingency Table")
knitr::kable(vonK_Over70_Table, caption = "Over 70 Contingency Table")
```
...
#### 13.17: Using the Italy and China data stratified by the age groups $<70$ and $\geq 70$, obtain empirical estimates of $P_1$ and $P_2$, as defined in an SDis. In the following questions, assume that $P_1$ and $P_2$ are the population values.
...
#### 13.18: Stratify the Italy and China data by the age groups $<50$ and $\geq 50$. Display the corresponding contingency tables. 
...
vonK_Aggregated_Table <- vonK_Data_50 |> select(-Age_Range) |> group_by(Country) |> summarise_all(.funs = c(sum)) |> adorn_totals("row")
vonK_Under50_Table <- vonK_Data_50 |> filter(Age_Range == "Under_50") |> select(-Age_Range) |> adorn_totals("row") 
vonK_Over50_Table <- vonK_Data_50 |> filter(Age_Range == "Over_50") |> select(-Age_Range) |> adorn_totals("row")
knitr::kable(vonK_Aggregated_Table, caption = "Aggregated Contingency Table")
knitr::kable(vonK_Under50_Table, caption = "Under 50 Contingency Table")
knitr::kable(vonK_Over50_Table, caption = "Over 50 Contingency Table")
```
